...
const HeroSection = () => {
  ...
  return (
    <section className="relative h-[500px] overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col justify-center items-center text-white px-4 h-full">
      </div>
        ...
        <Marquee gradient={false} speed={50}>
          {images.map((image, index) => (
            <div key={index} className="mx-2">
              <img src={image || "/placeholder.svg"} alt={`Gas station image ${index + 1}`} className="h-40 w-60 object-cover rounded-lg" />
            </div>
          ))}
        </Marquee>
      </div>
